...
func (s *Emitter) writeDelete(writer io.Writer, singlePartQuery *model.SinglePartQuery, delete *Delete) error {
	if delete.NodeDelete {
		if _, err := WriteStrings(writer, "delete from ", pgModel.NodeTable, " as ", delete.Binding.Symbol); err != nil {
			return err
		}
				...
				for _, pattern := range matchClause.Pattern {
					for _, patternElement := range pattern.PatternElements {
						if nodePattern, isNodePattern := patternElement.AsNodePattern(); isNodePattern {
							...
							if _, err := WriteStrings(writer, pgModel.NodeTable, " as "); err != nil {
								return err
							}
							...
							if _, err := WriteStrings(writer, pgModel.EdgeTable, " as "); err != nil {
								return err
							}
		...
		if _, err := WriteStrings(writer, "delete from ", pgModel.EdgeTable, " as ", delete.Binding.Symbol); err != nil {
			return err
		}
