...
with s0 as (select (n0.id, n0.kind_ids, n0.properties)::nodecomposite as n0 from node n0 where coalesce(n0.properties ->> 'objectid', '')::text like '%' || @pi0 and not (n0.kind_ids operator (pg_catalog.&&) array [1]::int2[] or n0.kind_ids operator (pg_catalog.&&) array [2]::int2[])) select s0.n0 as n from s0;
