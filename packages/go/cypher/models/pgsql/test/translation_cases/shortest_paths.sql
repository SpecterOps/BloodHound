...
-- pgsql_params:{"pi0":"insert into next_pathspace (root_id, next_id, depth, satisfied, is_cycle, path) select e0.end_id, e0.start_id, 1, n1.kind_ids operator (pg_catalog.\u0026\u0026) array [2]::int2[], e0.start_id = e0.end_id, array [e0.id] from edge e0 join node n0 on coalesce(n0.properties -\u003e\u003e 'system_tags', '')::text like '%admin_tier_0%' and n0.properties -\u003e\u003e 'name' ~ 'name.*' and n0.kind_ids operator (pg_catalog.\u0026\u0026) array [1]::int2[] and n0.id = e0.end_id join node n1 on n1.id = e0.start_id where e0.kind_id = any (array [3, 4]::int2[]);","pi1":"insert into next_pathspace (root_id, next_id, depth, satisfied, is_cycle, path) select s1.root_id, e0.start_id, s1.depth + 1, n1.kind_ids operator (pg_catalog.\u0026\u0026) array [2]::int2[], e0.id = any (s1.path), s1.path || e0.id from pathspace s1 join edge e0 on e0.end_id = s1.next_id and e0.kind_id = any (array [3, 4]::int2[]) join node n1 on n1.id = e0.start_id where s1.depth \u003c 10 and not s1.is_cycle;"}
with s0 as (with s1(root_id, next_id, depth, satisfied, is_cycle, path)
                     as (select * from asp_harness(@pi0::text, @pi1::text, 10))
            select (select array_agg((e0.id, e0.start_id, e0.end_id, e0.kind_id, e0.properties)::edgecomposite)
                    from edge e0
...
-- pgsql_params:{"pi0":"insert into next_pathspace (root_id, next_id, depth, satisfied, is_cycle, path) select e0.start_id, e0.end_id, 1, n1.id = 2, e0.start_id = e0.end_id, array [e0.id] from edge e0 join node n0 on n0.id = 1 and n0.id = e0.start_id join node n1 on n1.id = e0.end_id where e0.kind_id = any (array [1]::int2[]);","pi1":"insert into next_pathspace (root_id, next_id, depth, satisfied, is_cycle, path) select s1.root_id, e0.end_id, s1.depth + 1, n1.id = 2, e0.id = any (s1.path), s1.path || e0.id from pathspace s1 join edge e0 on e0.start_id = s1.next_id and e0.kind_id = any (array [1]::int2[]) join node n1 on n1.id = e0.end_id where s1.depth \u003c 10 and not s1.is_cycle;"}
with s0 as (with s1(root_id, next_id, depth, satisfied, is_cycle, path)
                     as (select * from asp_harness(@pi0::text, @pi1::text, 10))
            select (select array_agg((e0.id, e0.start_id, e0.end_id, e0.kind_id, e0.properties)::edgecomposite)
                    from edge e0
