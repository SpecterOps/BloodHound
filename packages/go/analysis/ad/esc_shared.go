...
func PostTrustedForNTAuth(ctx context.Context, db graph.Database, operation analysisOps.StatTrackedOperation[post.EnsureRelationshipJob]) error {
...
func PostIssuedSignedBy(operation analysisOps.StatTrackedOperation[post.EnsureRelationshipJob], enterpriseCertAuthorities []*graph.Node, rootCertAuthorities []*graph.Node, aiaCertAuthorities []*graph.Node) error {
	operation.Operation.SubmitReader(func(ctx context.Context, tx graph.Transaction, outC chan<- post.EnsureRelationshipJob) error {
...
func PostEnterpriseCAFor(operation analysisOps.StatTrackedOperation[post.EnsureRelationshipJob], enterpriseCertAuthorities []*graph.Node) error {
	operation.Operation.SubmitReader(func(ctx context.Context, tx graph.Transaction, outC chan<- post.EnsureRelationshipJob) error {
...
func PostExtendedByPolicyBinding(operation analysisOps.StatTrackedOperation[post.EnsureRelationshipJob], certTemplates []*graph.Node) error {
	operation.Operation.SubmitReader(func(ctx context.Context, tx graph.Transaction, outC chan<- post.EnsureRelationshipJob) error {
