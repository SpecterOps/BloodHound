	...
	var (
		...
		traversalPlan = ops.TraversalPlan{
			...
			BranchQuery: func() graph.Criteria {
				return query.KindIn(query.Relationship(), ad.Contains, ad.SameForestTrusted)
			},
...
func GetADCSESC13EdgeComposition(ctx context.Context, db graph.Database, edge *graph.Relationship) (graph.PathSet, error) {
	/*
		...
		MATCH p4 = (d)-[:Contains|SameForestTrusted*..]->(g)
		RETURN p1,p2,p3,p4
	*/
...
func adcsESC13Path4Pattern() traversal.PatternContinuation {
	return traversal.NewPattern().
		...
		InboundWithDepth(0, 0, query.Kind(query.Relationship(), ad.SameForestTrusted))
}
