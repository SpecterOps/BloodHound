...
func TestGetADCSESC3EdgeComposition(t *testing.T) {
	...
	if cfg, err := utils.LoadIntegrationTestConfig(); err != nil {
		t.Fatalf("%v", err)
	} else if _, graphDB, err := server.ConnectDatabases(cfg); err != nil {
		t.Fatalf("%v", err)
	} else {
		defer graphDB.Close()
		...
		require.Nil(t, graphDB.ReadTransaction(ctx, func(tx graph.Transaction) error {
			return tx.Relationships().Filter(query.KindIn(query.Relationship(), adSchema.ADCSESC1)).Fetch(func(cursor graph.Cursor[*graph.Relationship]) error {
		...
		paths, err := ad.GetADCSESC1EdgeComposition(ctx, graphDB, edges[0])
