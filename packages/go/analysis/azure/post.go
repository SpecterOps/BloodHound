...
func addMembers(roleAssignments RoleAssignments, operation analysis.StatTrackedOperation[analysis.CreatePostRelationshipJob]) {
	for tenantGroupID, tenantGroup := range roleAssignments.Principals.Get(azure.Group) {
		...
		if err := operation.Operation.SubmitReader(func(ctx context.Context, tx graph.Transaction, outC chan<- analysis.CreatePostRelationshipJob) error {
			...
			slog.Error(fmt.Sprintf("Failed to submit post processing job for users with role allowing AZAddMembers edge: %v", err))
		}
		...
		if err := operation.Operation.SubmitReader(func(ctx context.Context, tx graph.Transaction, outC chan<- analysis.CreatePostRelationshipJob) error {
			...
			slog.Error(fmt.Sprintf("Failed to submit post processing job for service principals with role allowing AZAddMembers edge: %v", err))
		}
