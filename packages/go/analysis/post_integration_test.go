...
func TestDeleteTransitEdges(t *testing.T) {
	...
	_, err := analysis.DeleteTransitEdges(context.Background(), testCtx.Graph.Database, "Delete Azure Post-Processed Relationships", graph.Kinds{ad.Entity, azure.Entity}, azure.PostProcessedRelationships()...)
	...
	_, err = analysis.DeleteTransitEdges(context.Background(), testCtx.Graph.Database, "Delete AD Post-Processed Relationships", graph.Kinds{ad.Entity, azure.Entity}, ad.PostProcessedRelationships()...)
	// Deleting transit edges must not return an error
	require.Nil(t, err)
