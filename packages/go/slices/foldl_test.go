...
func TestFoldl(t *testing.T) {
	...
	for _, c := range cases {
		...
		case Case[int, int]:
			t.Run(c.Name, func(t *testing.T) {
				require.Equal(t, c.Output, slices.Foldl(c.InitVal, c.List, c.Accumulator))
				require.Equal(t, c.Output, slices.FoldlLazy(c.InitVal, c.List, c.Accumulator))
			})
		}
	}
	...
	t.Run("should handle multiple reducers", func(t *testing.T) {
		// ((((5 +1)*1)+1)*1)+2)*2 = 18
		require.Equal(t, 18, slices.Foldl(5, []int{1, 1, 2}, sum, mult))
		require.Equal(t, 18, slices.FoldlLazy(5, []int{1, 1, 2}, sum, mult))
	})
}
...
func FuzzFoldl(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, a []byte, b []byte) {
		out := slices.Foldl(a, b, func(x []byte, y byte) []byte {
			return cons(x, y)
		})
...
func FuzzFoldlLazy(f *testing.F) {
	f.Add([]byte{5}, []byte{1, 2, 3, 4})
	f.Fuzz(func(t *testing.T, a []byte, b []byte) {
		out := slices.FoldlLazy(a, b, func(x []byte, y byte) []byte {
			return cons(x, y)
		})
		require.Equal(t, len(a)+len(b), len(out))
	})
