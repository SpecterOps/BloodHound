...
func Traversal(tx graph.Transaction, plan TraversalPlan, pathVisitors ...PathVisitor) error {
	...
	for len(stack) > 0 {
		...
		if pathTreeSize := rootSegment.SizeOf(); pathTreeSize > TraversalMemoryLimit {
			return fmt.Errorf("%w - Limit: %.2f MB - Memory In-Use: %.2f MB", ErrTraversalMemoryLimit, TraversalMemoryLimit.Mebibytes(), pathTreeSize.Mebibytes())
		}
