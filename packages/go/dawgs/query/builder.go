...
func (s *Builder) prepareMatch() error {
	var (
		...
		bindWalk = walk.NewVisitor(func(stack *walk.WalkStack, branch cypher.Expression) error {
	...
	for _, updatingClause := range s.regularQuery.SingleQuery.SinglePartQuery.UpdatingClauses {
		...
		case *cypher.Create:
			if err := walk.Walk(typedClause, walk.NewVisitor(func(stack *walk.WalkStack, element cypher.Expression) error {
		...
		case *cypher.Delete:
			if err := walk.Walk(typedClause, bindWalk); err != nil {
				return err
			}
		}
	}
	...
	if firstReadingClause := GetFirstReadingClause(s.regularQuery); firstReadingClause != nil && firstReadingClause.Match.Where != nil {
		if err := walk.Walk(firstReadingClause.Match.Where, bindWalk); err != nil {
			return err
		}
	}
	...
	if s.regularQuery.SingleQuery.SinglePartQuery.Return != nil {
		if err := walk.Walk(s.regularQuery.SingleQuery.SinglePartQuery.Return, bindWalk); err != nil {
			return err
		}
	}
