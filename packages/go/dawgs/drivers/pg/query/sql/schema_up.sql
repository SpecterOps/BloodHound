...
-- Traversal functions and schema

create or replace function public._format_traversal_where_clause(initial_criteria text, where_clause text) returns text as
$$
declare
  formatted_query text := '';
begin
  if length(initial_criteria) > 0 then
    if length(where_clause) > 0 then
      formatted_query := ' where ' || initial_criteria || ' and ' || where_clause;
    else
      formatted_query := ' where ' || initial_criteria;
    end if;
...
create or replace function public._format_traversal_query(exit_criteria text, cycle_criteria text,
                                                          where_criteria text default '',
                                                          initial_criteria text default '',
...
begin
  if bind_pathspace then
    formatted_query :=
      formatted_query || 'select p.path || e.id, e.end_id, ' || exit_criteria || ', ' || cycle_criteria ||
      ' from edge e join pathspace_current p on p.next = e.start_id';
  else
    formatted_query := formatted_query || 'select array [e.id]::int4[], e.end_id, ' || exit_criteria || ', ' ||
                       cycle_criteria ||
                       ' from edge e';
  end if;

  if bind_start then
    formatted_query := formatted_query || ' join node sn on sn.id = e.start_id';
  end if;

  if bind_end then
    formatted_query := formatted_query || ' join node en on en.id = e.end_id ';
  end if;

  return formatted_query || _format_traversal_where_clause(initial_criteria, where_criteria) || ';';
end;
$$
  language plpgsql immutable
                   parallel safe
...
create or replace function public.traverse(start_node int4, where_criteria text, exit_criteria text, max_depth int4,
                                           bind_start bool default false,
                                           bind_end bool default false)
  returns table
          (
            path int4[]
          )
...
begin
  ...
  execute _format_traversal_query(exit_criteria := exit_criteria,
                                  cycle_criteria := 'e.start_id = e.end_id',
                                  bind_pathspace := false,
                                  bind_start := bind_start,
                                  bind_end := bind_end,
                                  initial_criteria := 'e.start_id = ' || start_node,
  ...
  while depth < max_depth and (select count(*) from pathspace_current p where p.is_terminal) = 0
    loop
      ...
      execute _format_traversal_query(exit_criteria := exit_criteria,
                                      cycle_criteria := 'e.id = any (p.path)',
                                      bind_pathspace := true,
                                      bind_start := bind_start,
                                      bind_end := bind_end,
                                      where_criteria := where_criteria);
...
select array_agg(distinct (n.id, n.kind_ids, n.properties)::nodeComposite),
       array_agg(distinct (e.id, e.start_id, e.end_id, e.kind_id, e.properties)::edgeComposite)
from edge e
       join node n on n.id = e.start_id or n.id = e.end_id
where e.id = any (path);
$$
  language sql
  immutable
...
create or replace function public.traverse_paths(
  start_node int4, where_criteria text, exit_criteria text, max_depth int4) returns setof pathComposite as
$$
declare
  path int4[];
begin
  for path in select t.path from traverse(start_node, where_criteria, exit_criteria, max_depth) as t
    loop
      return next edges_to_path(variadic path);
    end loop;
