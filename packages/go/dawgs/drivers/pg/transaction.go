...
func (s *transaction) CreateNode(properties *graph.Properties, kinds ...graph.Kind) (*graph.Node, error) {
	if graphTarget, err := s.getTargetGraph(); err != nil {
		...
		var (
			nodeID int32
			result = s.driver().QueryRow(s.ctx, createNodeStatement, s.queryExecMode, graphTarget.ID, kindIDSlice, propertiesJSONB)
		)
...
func (s *transaction) UpdateNode(node *graph.Node) error {
	...
	return s.Nodes().Filter(query.Equals(query.NodeID(), node.ID)).Exec(updateStatements...)
}
...
func (s *transaction) CreateRelationshipByIDs(startNodeID, endNodeID graph.ID, kind graph.Kind, properties *graph.Properties) (*graph.Relationship, error) {
	if graphTarget, err := s.getTargetGraph(); err != nil {
		...
		var (
			edgeID int32
			result = s.driver().QueryRow(s.ctx, createEdgeStatement, s.queryExecMode, graphTarget.ID, startNodeID, endNodeID, kindIDSlice[0], propertiesJSONB)
		)
...
func (s *transaction) query(query string, parameters map[string]any) (pgx.Rows, error) {
	if parameters == nil || len(parameters) == 0 {
		return s.driver().Query(s.ctx, query, s.queryExecMode)
	}

	return s.driver().Query(s.ctx, query, s.queryExecMode, pgx.NamedArgs(parameters))
}
