...
build='build.10')
    ...
    def compare(self, other: Comparable) -> int:
        ...
        if isinstance(other, String.__args__):  # type: ignore
            ...
            raise TypeError(
                f"Expected str, bytes, dict, tuple, list, or {cls.__name__} instance, " f"but got {type(other)}"
            )
    ...
    def next_version(self, part: str, prerelease_token: str = "rc") -> "Version":
        ...
        if part not in validparts:
            raise ValueError(f"Invalid part. Expected one of {validparts}, but got {part!r}")
        version = self
    ...
    def __getitem__(self, index: Union[int, slice]) -> Union[int, Optional[str], Tuple[Union[int, str], ...]]:
        ...
        part = tuple(filter(lambda p: p is not None, cast(Iterable, self.to_tuple()[index])))
...
prerelease='pre.2', build='build.4')
    ...
    def is_compatible(self, other: "Version") -> bool:
        ...
        * both majors are equal and higher than 0. The minor of b's
        ...
        return (self.major == other.major) and (other.minor >= self.minor) and (self.prerelease == other.prerelease)
