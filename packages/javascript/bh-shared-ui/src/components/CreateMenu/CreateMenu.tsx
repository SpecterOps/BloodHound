...
const CreateMenu: React.FC<{
    ...
    const menuOrButton = menuItems.length > 1 ? 
    ...
    if (featureFlag != undefined && !!featureFlagEnabledMenuItems) {
        const featureFlagEnabledMenuOrButton = featureFlagEnabledMenuItems.length > 1 ? 
        ...
        return (
            <FeatureFlag
                flagKey={featureFlag}
                enabled={featureFlagEnabledMenuOrButton}
                disabled={menuOrButton}
            />
        );
    } else {
        return menuOrButton;
