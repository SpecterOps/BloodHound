...
4. The `AxeResults` are handed to `expectNoAccessibilityViolations(testInfo, results, opts?)`, which attaches `axe-results.json` (always), `a11y-violations.md` when violations exist, and `a11y-incomplete.md` when checks require manual review. Violations fail by default.
