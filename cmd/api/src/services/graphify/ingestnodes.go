...
func IngestNode(ic *IngestContext, baseKind graph.Kind, nextNode ein.IngestibleNode) error {
	var (
		...
		normalizedProperties = normalizeEinNodeProperties(nextNode.PropertyMap, nextNode.ObjectID, ic.IngestTime, ic.PreserveObjectIdentifierCasing)
...
func normalizeEinNodeProperties(properties map[string]any, objectID string, ingestTime time.Time, preserveObjectIdentifierCasing bool) map[string]any {
	if properties == nil {
