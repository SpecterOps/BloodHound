...
func IngestWrapper(batch *IngestContext, reader io.ReadSeeker, meta ingest.LegacyMetadata, readOpts ReadOptions) error {
...
type basicIngestHandler func(batch *IngestContext, reader io.ReadSeeker, meta ingest.LegacyMetadata) error
...
type sourceKindIngestHandler func(batch *IngestContext, reader io.ReadSeeker, meta ingest.LegacyMetadata, register registrationFn) error

func defaultBasicHandler[T any](conversionFunc ConversionFuncWithTime[T]) basicIngestHandler {
	return func(batch *IngestContext, reader io.ReadSeeker, meta ingest.LegacyMetadata) error {
...
var basicHandlers = map[ingest.DataType]basicIngestHandler{
	ingest.DataTypeComputer: func(batch *IngestContext, reader io.ReadSeeker, meta ingest.LegacyMetadata) error {
	...
	ingest.DataTypeGroup: func(batch *IngestContext, reader io.ReadSeeker, meta ingest.LegacyMetadata) error {
		if decoder, err := getDefaultDecoder(reader); err != nil {
			return err
		} else {
	...
	ingest.DataTypeSession: func(batch *IngestContext, reader io.ReadSeeker, meta ingest.LegacyMetadata) error {
		if decoder, err := getDefaultDecoder(reader); err != nil {
			return err
		} else {
	...
	ingest.DataTypeAzure: func(batch *IngestContext, reader io.ReadSeeker, meta ingest.LegacyMetadata) error {
		if decoder, err := getDefaultDecoder(reader); err != nil {
			return err
		} else {
...
var sourceKindHandlers = map[ingest.DataType]sourceKindIngestHandler{
	ingest.DataTypeOpenGraph: func(batch *IngestContext, reader io.ReadSeeker, meta ingest.LegacyMetadata, registerSourceKind registrationFn) error {
