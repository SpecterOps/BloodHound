...
type FileService interface {
	...
	WriteTempFile(ctx context.Context, prefix string, data []byte, opts WriteOptions) (string, error)
}
...
func (s *LocalFileService) WriteTempFile(ctx context.Context, prefix string, data []byte, opts WriteOptions) (string, error) {
	id, err := randomID()
	if err != nil {
		return "", err
	}
	...
	if err := s.Storage.Put(ctx, tempPath, bytes.NewReader(data), opts); err != nil {
		return "", err
	}
