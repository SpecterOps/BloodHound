...
func (s *JobService) FailAnalyzedIngestJobs() {
	...
	if ingestJobsUnderAnalysis, err := s.db.GetIngestJobsWithStatus(s.ctx, model.JobStatusAnalyzing); err != nil {
		...
		for _, job := range ingestJobsUnderAnalysis {
			if err := s.updateIngestJobStatus(job, model.JobStatusFailed, "Analysis failed"); err != nil {
...
func (s *JobService) PartialCompleteIngestJobs() {
	...
	if ingestJobsUnderAnalysis, err := s.db.GetIngestJobsWithStatus(s.ctx, model.JobStatusAnalyzing); err != nil {
		...
		for _, job := range ingestJobsUnderAnalysis {
			if err := s.updateIngestJobStatus(job, model.JobStatusPartiallyComplete, "Partially Completed"); err != nil {
...
func (s *JobService) CompleteAnalyzedIngestJobs() {
	...
	if ingestJobsUnderAnalysis, err := s.db.GetIngestJobsWithStatus(s.ctx, model.JobStatusAnalyzing); err != nil {
		...
		for _, job := range ingestJobsUnderAnalysis {
			...
			if err := s.updateIngestJobStatus(job, status, message); err != nil {
...
func (s *JobService) ProcessFinishedIngestJobs() {
	...
	// This is yucky. Eventually this should be teased out to make our job service as generic as we can.
	if jobs, err := s.db.GetIngestJobsWithStatus(s.ctx, model.JobStatusIngesting); err != nil {
		...
		for _, job := range jobs {
			if remainingIngestTasks, err := s.db.GetIngestTasksForJob(s.ctx, job.ID); err != nil {
				...
				if err := s.updateIngestJobStatus(job, model.JobStatusAnalyzing, "Analyzing"); err != nil {
					slog.ErrorContext(s.ctx, fmt.Sprintf("Error updating ingest job %d: %v", job.ID, err))
				}
			}
		}
	...
	if jobs, err := s.db.GetIngestJobsWithStatus(s.ctx, model.JobStatusRunning); err != nil {
		...
		for _, job := range jobs {
			if job.LastIngest.Before(threshold) {
				...
				if err := s.timeOutIngestJob(job.ID, fmt.Sprintf("Ingest timeout: No ingest activity observed in %f minutes. Upload incomplete.", now.Sub(threshold).Minutes())); err != nil {
...
func (s *JobService) timeOutIngestJob(jobID int64, message string) error {
	if job, err := s.db.GetIngestJob(s.ctx, jobID); err != nil {
		...
		return s.db.UpdateIngestJob(s.ctx, job)
	}
}

func (s *JobService) updateIngestJobStatus(job model.IngestJob, status model.JobStatus, message string) error {
	job.Status = status
	job.StatusMessage = message
	job.EndTime = time.Now().UTC()
...
// These are called by the API directly. These don't feel right here, but it also feel icky to move
// them up to the handler. Not sure exactly what to do with these. Should the handler created a service?
// Given the lifecycle of a request context that may not make the most sense.
