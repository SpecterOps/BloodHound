...
func NewPipeline(ctx context.Context, cfg config.Configuration, db database.Database, graphDB graph.Database, cache cache.Cache, ingestSchema upload.IngestSchema) *Pipeline {
	return &Pipeline{
		db:                  db,
		graphdb:             graphDB,
		cache:               cache,
...
func (s *Pipeline) clearOrphanedData() {
...
func (s *Pipeline) analyze() {
	...
	if hasJobsWaitingForAnalysis, err := s.jobService.HasIngestJobsWaitingForAnalysis(); err != nil {
		slog.ErrorContext(s.ctx, fmt.Sprintf("Failed looking up jobs waiting for analysis: %v", err))
