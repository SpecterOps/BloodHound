...
func FetchNodesFromSeeds(ctx context.Context, graphDb graph.Database, seeds []model.SelectorSeed, expansionMethod model.AssetGroupExpansionMethod, limit int) nodeWithSourceSet {
	...
	if expansionMethod == model.AssetGroupExpansionMethodAll || expansionMethod == model.AssetGroupExpansionMethodChildren {
		collected := fetchExpandedNodes(ctx, graphDb, fetchChildNodes, seedNodes, result, limit)
		if result.LimitReached(limit) {
			return result
		}
	...
	if expansionMethod == model.AssetGroupExpansionMethodAll || expansionMethod == model.AssetGroupExpansionMethodParents {
		_ = fetchExpandedNodes(ctx, graphDb, fetchParentNodes, seedNodes, result, limit)
	}
...
func fetchChildNodes(ctx context.Context, tx traversal.Traversal, node *graph.Node, ch chan<- *nodeWithSource) {
...
func fetchParentNodes(ctx context.Context, tx traversal.Traversal, node *graph.Node, ch chan<- *nodeWithSource) {
	if node.Kinds.ContainsOneOf(ad.Entity) {
		if err := fetchADParentNodes(ctx, tx, node, ch); err != nil {
		...
		if err := fetchAzureParentNodes(ctx, tx, node, ch); err != nil {
...
// fetchExpandedNodes - concurrently fetches expanded nodes based off seed nodes
func fetchExpandedNodes(ctx context.Context, db graph.Database, fetchNodes func(ctx context.Context, tx traversal.Traversal, node *graph.Node, ch chan<- *nodeWithSource), seedNodes nodeWithSourceSet, result nodeWithSourceSet, limit int) []*nodeWithSource {
	var (
		wg                      = sync.WaitGroup{}
	...
	defer doneFunc()
	// Spin out a job per seed node
	for _, node := range seedNodes {
		wg.Add(1)
		go func() {
			defer wg.Done()
			fetchNodes(ctxWithCancel, traversalInst, node.Node, ch)
		}()
	}
	...
	// This will block and collect all expanded nodes until channel is closed
