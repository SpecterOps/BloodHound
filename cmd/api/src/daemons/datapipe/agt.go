...
func fetchNodesFromSeeds(ctx context.Context, graphDb graph.Database, seeds []model.SelectorSeed) (graph.ThreadSafeNodeSet, error) {
	...
	for _, node := range seedNodes {
		...
		case node.Kinds.ContainsOneOf(ad.Group, azure.Group):
			err = expandGroupNode(ctx, traversalInst, node, result)

		case node.Kinds.ContainsOneOf(ad.OU, ad.Container):
			err = expandContainerOrOU(ctx, traversalInst, node, result)
		default:
...
func expandGroupNode(ctx context.Context, tx traversal.Traversal, node *graph.Node, result *graph.ThreadSafeNodeSet) error {
	// Groups expand through MemberOf edges
	if err := tx.BreadthFirst(ctx, traversal.Plan{
		...
		Driver: traversal.NewPattern().InboundWithDepth(0, 0, query.And(
			...
			result.AddIfNotExists(path.Node)
...
func expandContainerOrOU(ctx context.Context, tx traversal.Traversal, node *graph.Node, result *graph.ThreadSafeNodeSet) error {
	// OU / Containers expand through Contains edges
	if err := tx.BreadthFirst(ctx, traversal.Plan{
		Root: node,
		Driver: traversal.NewPattern().OutboundWithDepth(0, 0, query.And(
		...
		if selectedNodes, err = db.GetSelectorNodesBySelectorIds(ctx, selectorIds...); err != nil {
			...
			for _, nodeDb := range selectedNodes {
				if !nodesSeen.Contains(nodeDb.NodeId.Uint64()) {
					// Skip any that are not certified when tag requires certification
