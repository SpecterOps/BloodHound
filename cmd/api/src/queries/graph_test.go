...
func TestGraphQuery_PrepareCypherQuery(t *testing.T) {
	...
	t.Run("invalid cypher", func(t *testing.T) {
		_, err := gq.PrepareCypherQuery(rawCypherInvalid, queries.MaxQueryComplexityWeightAllowed)
	...
	t.Run("valid cypher with mutation while mutations disabled", func(t *testing.T) {
		_, err := gqMutDisable.PrepareCypherQuery(rawCypherMutation, queries.MaxQueryComplexityWeightAllowed)
		assert.ErrorContains(t, err, "not supported")
	})

	t.Run("valid cypher without mutation", func(t *testing.T) {
		preparedQuery, err := gq.PrepareCypherQuery(rawCypherRead, queries.MaxQueryComplexityWeightAllowed)
		require.Nil(t, err)
		assert.Equal(t, preparedQuery.HasMutation, false)
	})

	t.Run("valid cypher with mutation", func(t *testing.T) {
		preparedQuery, err := gq.PrepareCypherQuery(rawCypherMutation, queries.MaxQueryComplexityWeightAllowed)
		require.Nil(t, err)
		assert.Equal(t, preparedQuery.HasMutation, true)
	})

	t.Run("valid cypher pathfinding with expansion", func(t *testing.T) {
		preparedQuery, err := gq.PrepareCypherQuery(rawCypherPathfindingExpansion, queries.MaxQueryComplexityWeightAllowed)
		require.Nil(t, err)
		assert.Equal(t, preparedQuery.HasMutation, false)
	})

	t.Run("valid cypher without mutation with expansion", func(t *testing.T) {
		preparedQuery, err := gq.PrepareCypherQuery(rawCypherReadExpansion, queries.MaxQueryComplexityWeightAllowed)
		require.Nil(t, err)
		assert.Equal(t, preparedQuery.HasMutation, false)
	})

	t.Run("valid cypher with creation and expansion", func(t *testing.T) {
		_, err := gq.PrepareCypherQuery(rawCypherCreationAndExpansion, queries.MaxQueryComplexityWeightAllowed)
		assert.ErrorContains(t, err, "not supported")
	})

	t.Run("valid cypher with deletion and expansion", func(t *testing.T) {
		_, err := gq.PrepareCypherQuery(rawCypherDeleteAndExpansion, queries.MaxQueryComplexityWeightAllowed)
		assert.ErrorContains(t, err, "not supported")
	})
	t.Run("valid cypher with updates and expansion", func(t *testing.T) {
		_, err := gq.PrepareCypherQuery(rawCypherUpdateAndExpansion, queries.MaxQueryComplexityWeightAllowed)
		assert.ErrorContains(t, err, "not supported")
	})
...
func TestGraphQuery_RawCypherQuery(t *testing.T) {
	...
	t.Run("RawCypherQuery user set timeouts successfully", func(t *testing.T) {
		...
		preparedQuery, err := gq.PrepareCypherQuery("match (n:Label) return n;", queries.MaxQueryComplexityWeightAllowed)
		require.Nil(t, err)
	...
	t.Run("RawCypherQuery query complexity controls", func(t *testing.T) {
		...
		preparedQuery, err := gq.PrepareCypherQuery("match ()-[:HasSession*..]->()-[:MemberOf*..]->() return n;", queries.MaxQueryComplexityWeightAllowed)
		require.Nil(t, err)
		_, err = gq.RawCypherQuery(outerBHCtxInst.ConstructGoContext(), preparedQuery, false)
		require.Nil(t, err)
	...
	t.Run("RawCypherQuery read query leverages read tx", func(t *testing.T) {
		...
		preparedQuery, err := gq.PrepareCypherQuery("match (b) where b.name = 'harley' return b;", queries.MaxQueryComplexityWeightAllowed)
		require.Nil(t, err)
	...
	t.Run("RawCypherQuery mutation query leverages write tx if enable_cypher_mutations is true", func(t *testing.T) {
		...
		preparedQuery, err := qgWMut.PrepareCypherQuery("match (b) where b.name = 'bruce' remove b.prop return b;", queries.MaxQueryComplexityWeightAllowed)
		require.Nil(t, err)
