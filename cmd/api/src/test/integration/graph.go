...
var enlistedDrivers = []string{
	neo4j.DriverName,
	pg.DriverName,
}

func driverConfig(driverName string, cfg config.Configuration) (dawgs.Config, error) {
	var dawgsCfg dawgs.Config
	...
	default:
		return dawgsCfg, fmt.Errorf("unknown dawgs driver: %s", driverName)
	}
}
...
func NewGraphTestContext(testCtrl test.Controller, schema graph.Schema) *GraphTestContext {
	...
	if dawgsCfg, err := driverConfig(pg.DriverName, LoadConfiguration(testCtrl)); err != nil {
		testCtrl.Fatalf("Unable to instantiate dawgs configuration: %v", err)
		return nil
	} else {
		return &GraphTestContext{
			testCtx: testCtx,
			Graph:   NewGraphContext(testCtx, pg.DriverName, dawgsCfg, schema),
		}
	}
}
