...
func (s *GraphContext) End(t test.Context) {
	if err := s.Database.Close(t); err != nil {
		// TODO: Log any closing errors
	}
}

func NewGraphContext(ctx test.Context, driverName string, config dawgs.Config, schema graph.Schema) *GraphContext {
	graphContext := &GraphContext{
		schema: schema,
	}

	if graphDB, err := dawgs.Open(ctx, driverName, config); err != nil {
		ctx.Fatalf("Unable to open a connection to the graph database: %v", err)
	} else {
		graphContext.Database = graphDB
