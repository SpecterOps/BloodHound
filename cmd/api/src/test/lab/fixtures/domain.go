...
func NewDomainFixture(sid uuid.UUID, name string, collected, blocksInheritance bool, apiClient *lab.Fixture[apiclient.Client]) *lab.Fixture[*graph.Node] {
	...
	if err := lab.SetDependency(fixture, apiClient); err != nil {
		log.Fatalln(err)
	}
...
var (
	...
	BasicDomainFixture = NewDomainFixture(BasicDomainSID, BasicDomainName, true, false, BHAdminApiClientFixture)
)
