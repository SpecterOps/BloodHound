...
func registerV2Auth(cfg config.Configuration, db database.Database, permissions auth.PermissionSet, routerInst *router.Router, authenticator api.Authenticator) {
	...
	router.With(middleware.DefaultRateLimitMiddleware,
		...
		routerInst.GET("/api/v2/sso/{sso_provider_name}/login", managementResource.SSOLoginHandler).CheckFeatureFlag(db, appcfg.FeatureOIDCSupport),
		routerInst.GET("/api/v2/sso/{sso_provider_name}/callback", managementResource.SSOCallbackHandler).CheckFeatureFlag(db, appcfg.FeatureOIDCSupport),
