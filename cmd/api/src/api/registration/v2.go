...
func registerV2Auth(cfg config.Configuration, db database.Database, permissions auth.PermissionSet, routerInst *router.Router, authenticator api.Authenticator) {
	...
	router.With(middleware.DefaultRateLimitMiddleware,
		...
		routerInst.DELETE(fmt.Sprintf("/api/v2/sso-providers/oidc/{%s}", api.URIPathVariableOIDCProviderID), managementResource.DeleteOIDCProvider).CheckFeatureFlag(db, appcfg.FeatureOIDCSupport).RequirePermissions(permissions.AuthManageProviders),
