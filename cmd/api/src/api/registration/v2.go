...
func registerV2Auth(resources v2.Resources, routerInst *router.Router, permissions auth.PermissionSet) {
	...
	router.With(func() mux.MiddlewareFunc {
		...
		routerInst.GET("/api/v2/bloodhound-users-minimal", managementResource.ListActiveUsersMinimal).RequireAtLeastOnePermission(permissions.AuthReadUsers, permissions.AuthReadUsersMinimal), // returns user data without any sensitive information.
