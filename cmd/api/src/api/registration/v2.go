...
func NewV2API(resources v2.Resources, routerInst *router.Router) {
	...
	router.With(middleware.DefaultRateLimitMiddleware,
		...
		routerInst.PATCH(fmt.Sprintf("/api/v2/asset-group-tags/{%s}/selectors", api.URIPathVariableAssetGroupTagID), resources.CreateAssetGroupTagSelector).CheckFeatureFlag(resources.DB, appcfg.FeatureTierManagement).RequirePermissions(permissions.GraphDBWrite),
