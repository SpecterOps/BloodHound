...
func NewV2API(resources v2.Resources, routerInst *router.Router) {
	...
	router.With(middleware.DefaultRateLimitMiddleware,
		...
		// Asset group API
		routerInst.GET(fmt.Sprintf("/api/v2/asset-group-labels/{%s}", api.URIPathVariableTierOrLabelID), resources.GetTierOrLabelDetails).RequirePermissions(permissions.GraphDBRead),
	)
}
