...
func NewV2API(resources v2.Resources, routerInst *router.Router) {
	...
	router.With(func() mux.MiddlewareFunc {
		...
		routerInst.DELETE(fmt.Sprintf("/api/v2/asset-group-tags/{%s}", api.URIPathVariableAssetGroupTagID), resources.SoftDeleteAssetGroupTag).CheckFeatureFlag(resources.DB, appcfg.FeatureTierManagement).RequirePermissions(permissions.GraphDBWrite),
