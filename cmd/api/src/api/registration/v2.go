...
func NewV2API(resources v2.Resources, routerInst *router.Router) {
	...
	router.With(func() mux.MiddlewareFunc {
		...
		routerInst.POST(fmt.Sprintf("/api/v2/asset-group-tags/{%s}/members/certification", api.URIPathVariableAssetGroupTagID), resources.CertifyMembers).CheckFeatureFlag(resources.DB, appcfg.FeatureTierManagement).RequirePermissions(permissions.GraphDBRead),
