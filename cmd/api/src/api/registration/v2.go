...
func NewV2API(resources v2.Resources, routerInst *router.Router) {
	...
	router.With(func() mux.MiddlewareFunc {
		...
		routerInst.GET(fmt.Sprintf("/api/v2/data-quality-stats"), resources.GetAzureDataQualityStats).RequirePermissions(permissions.GraphDBRead).SupportsETAC(resources.DB, resources.DogTags),
