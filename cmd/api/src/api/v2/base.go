...
import (
	...
	ad1 "github.com/specterops/bloodhound/graphschema/ad"
)

var baseQueries = map[string]any{
...
func (s *Resources) GetBaseEntityInfo(response http.ResponseWriter, request *http.Request) {
	if hydrateCounts, err := api.ParseOptionalBool(request.URL.Query().Get(api.QueryParameterHydrateCounts), true); err != nil {
	...
	} else if node, err := s.GraphQuery.GetEntityByObjectId(request.Context(), objectId, ad1.Entity); err != nil {
		...
		results := s.GraphQuery.GetEntityCountResults(request.Context(), node, baseQueries)
