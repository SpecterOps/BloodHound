...
var ntauthstoreQueries = map[string]any{
...
func (s *Resources) GetNTAuthStoreEntityInfo(response http.ResponseWriter, request *http.Request) {
	if hydrateCounts, err := api.ParseOptionalBool(request.URL.Query().Get(api.QueryParameterHydrateCounts), true); err != nil {
	...
	} else if node, err := s.GraphQuery.GetEntityByObjectId(request.Context(), objectId, ad.NTAuthStore); err != nil {
		...
		results := s.GraphQuery.GetEntityCountResults(request.Context(), node, ntauthstoreQueries)
