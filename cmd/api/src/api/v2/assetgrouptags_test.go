...
func TestResources_CreateAssetGroupTag(t *testing.T) {
	var (
		mockCtrl      = gomock.NewController(t)
		mockDB        = mocks_db.NewMockDatabase(mockCtrl)
		resourcesInst = v2.Resources{
	...
	apitest.
		NewHarness(t, resourcesInst.CreateAssetGroupTag).
		Run([]apitest.Case{
			{
				Name: "Bad Request - Error Decoding Body",
				Input: func(input *apitest.Input) {
			...
			},
			{
				...
				Input: func(input *apitest.Input) {
					apitest.SetContext(input, userCtx)
					apitest.BodyStruct(input, model.AssetGroupTag{Type: 1})
				},
				Test: func(output apitest.Output) {
					apitest.StatusCode(output, http.StatusBadRequest)
			...
			},
			{
				...
				Input: func(input *apitest.Input) {
					apitest.SetContext(input, userCtx)
					apitest.BodyStruct(input, model.AssetGroupTag{
						Name:        "New Label",
						Type:        2,
				...
				Setup: func() {
					mockDB.EXPECT().
						CreateAssetGroupTag(gomock.Any(), model.AssetGroupTagTypeLabel, user, "New Label", "creating a new label", null.Int32{}, null.Bool{}).
						Return(model.AssetGroupTag{}, errors.New("db error"))
			...
			},
			{
				...
				Setup: func() {
					mockDB.EXPECT().
						CreateAssetGroupTag(gomock.Any(), model.AssetGroupTagTypeTier, user, "New Tier", "creating a new tier", null.Int32{}, null.BoolFrom(true)).
						Return(model.AssetGroupTag{
							Name:           "New Tier",
							Type:           1,
							Description:    "creating a new tier",
