...
func Test_SetNodeProperties(t *testing.T) {
	...
	tests := []struct {
			...
			nodes: []*graph.Node{
				{
					Properties: &graph.Properties{
						Map: map[string]any{
							...
							common.Collected.String():    true,
							common.PrimaryKind.String():  "OpenGraphEnvironment",
		...
		{
			name: "opengraph environment with cross-extension source kind is not classified as built in",
			nodes: []*graph.Node{
				{
					Properties: &graph.Properties{
						Map: map[string]any{
							common.ObjectID.String():     "4",
							common.Name.String():         "Node4",
							common.Collected.String():    true,
							common.PrimaryKind.String():  "OpenGraphEnvironment",
							graphschema.EnvironmentIDKey: "environment-4",
						},
					},
					Kinds: graph.Kinds{ad.Domain, graph.StringKind("OpenGraphEnvironment")},
				},
			},
			expected: model.EnvironmentSelectors{
				{
		...
		{
			name: "opengraph environment with built in source environment uses built in environment name",
			nodes: []*graph.Node{
				{
					Properties: &graph.Properties{
						Map: map[string]any{
							common.ObjectID.String():  "domain-id",
							common.Name.String():      "TESTLAB.LOCAL",
							common.Collected.String(): true,
							ad.DomainSID.String():     "domain-id",
						},
					},
					Kinds: graph.Kinds{ad.Domain, ad.Entity},
				},
				{
					Properties: &graph.Properties{
						Map: map[string]any{
							common.ObjectID.String():     "og-environment-id",
							common.Name.String():         "External Exposure - TESTLAB.LOCAL",
							common.Collected.String():    true,
							common.PrimaryKind.String():  "OpenGraphEnvironment",
							graphschema.EnvironmentIDKey: "domain-id",
						},
					},
					Kinds: graph.Kinds{ad.Domain, graph.StringKind("OpenGraphEnvironment")},
				},
		...
		{
			name: "opengraph principal with cross-extension source kind is not classified as built in",
			nodes: []*graph.Node{
				{
					Properties: &graph.Properties{
						Map: map[string]any{
							common.ObjectID.String():    "5",
							common.Name.String():        "Node5",
							common.Collected.String():   true,
							common.PrimaryKind.String(): "OpenGraphPrincipal",
						},
					},
					Kinds: graph.Kinds{ad.Domain, graph.StringKind("OpenGraphPrincipal")},
				},
			},
			expected: model.EnvironmentSelectors{
				{
		...
		{
			name: "generic domain with base primary kind is not classified as built in",
			nodes: []*graph.Node{
				{
					Properties: &graph.Properties{
						Map: map[string]any{
							common.ObjectID.String():    "domain-id",
							common.Name.String():        "Domain Node",
							common.Collected.String():   true,
							common.PrimaryKind.String(): ad.Entity.String(),
							ad.DomainSID.String():       "domain-id",
						},
					},
					Kinds: graph.Kinds{ad.Domain, ad.Entity},
		...
		{
			name: "generic user with domain source kind is not classified as built in",
			nodes: []*graph.Node{
				{
					Properties: &graph.Properties{
						Map: map[string]any{
							common.ObjectID.String():    "user-id",
							common.Name.String():        "User Node",
							common.Collected.String():   true,
							common.PrimaryKind.String(): ad.Entity.String(),
							ad.DomainSID.String():       "domain-id",
						},
					},
					Kinds: graph.Kinds{ad.Domain, ad.Entity, ad.User},
				},
			},
			expected: model.EnvironmentSelectors{
				{
