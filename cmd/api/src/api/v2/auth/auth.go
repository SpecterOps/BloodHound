...
func (s ManagementResource) CreateAuthToken(response http.ResponseWriter, request *http.Request) {
	...
	} else if authToken, err := auth.NewUserAuthToken(uuid.NullUUID{UUID: user.ID, Valid: true}, createUserTokenRequest.UserID, createUserTokenRequest.TokenName, auth.HMAC_SHA2_256); err != nil {
		api.WriteErrorResponse(request.Context(), api.BuildErrorResponse(http.StatusInternalServerError, api.ErrorResponseDetailsInternalServerError, request), response)
