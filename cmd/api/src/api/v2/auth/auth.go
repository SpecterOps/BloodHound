...
func (s ManagementResource) CreateUser(response http.ResponseWriter, request *http.Request) {
	...
	if err := api.ReadJSONRequestPayloadLimited(&createUserRequest, request); err != nil {
		...
		userTemplate.ExploreEnabled = true
		if etacFeatureFlag, err := s.db.GetFlagByKey(request.Context(), appcfg.FeatureETAC); err != nil {
			...
			if err := handleETACRequest(request.Context(), createUserRequest.UpdateUserRequest, roles, &userTemplate, s.GraphQuery, s.db); err != nil {
				api.WriteErrorResponse(request.Context(), api.BuildErrorResponse(http.StatusBadRequest, err.Error(), request), response)
				return
			}
		}
...
func (s ManagementResource) UpdateUser(response http.ResponseWriter, request *http.Request) {
	...
	if userID, err := uuid.FromString(rawUserID); err != nil {
		...
		if etacFeatureFlag, err := s.db.GetFlagByKey(request.Context(), appcfg.FeatureETAC); err != nil {
			...
			if err := handleETACRequest(request.Context(), updateUserRequest, effectiveRoles, &user, s.GraphQuery, s.db); err != nil {
				api.WriteErrorResponse(request.Context(), api.BuildErrorResponse(http.StatusBadRequest, err.Error(), request), response)
				return
			}
		}
