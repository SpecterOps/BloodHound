...
type AssetGroupTagData interface {
	...
	GetAssetGroupTagForButterfly(ctx context.Context) ([]model.AssetGroupTag, error)
}
...
func (s *BloodhoundDB) GetAssetGroupTagForButterfly(ctx context.Context) ([]model.AssetGroupTag, error) {
	var tags model.AssetGroupTags
	if result := s.db.WithContext(ctx).Raw(
		fmt.Sprintf(
			"SELECT id, type, kind_id, name, description, created_at, created_by, updated_at, updated_by, position, require_certify FROM %s WHERE type = 1 AND deleted_at IS NULL ORDER BY position ASC",
