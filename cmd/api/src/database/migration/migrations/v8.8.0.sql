...
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'schema_relationship_findings') THEN
    INSERT INTO schema_findings (type, schema_extension_id, kind_id, environment_id, name, display_name, created_at)
    SELECT 1, schema_extension_id, relationship_kind_id, environment_id, name, display_name, created_at
...
BEGIN
    IF NOT EXISTS (
      ...
      WHERE constraint_name = 'schema_remediations_schema_finding_id_fkey'
      AND table_name = 'schema_remediations'
    ) THEN
  ALTER TABLE schema_remediations ADD CONSTRAINT schema_remediations_schema_finding_id_fkey
      FOREIGN KEY (finding_id) REFERENCES schema_findings(id) ON DELETE CASCADE;
END IF;
END $$;
