...
func TestManifest_AddMigration(t *testing.T) {
	var (
		migration1 = migration.Migration{Filename: "/migration/1", Version: semver.New(0, 1, 0, "", "")}
		migration2 = migration.Migration{Filename: "/migration/2", Version: semver.New(1, 0, 0, "", "")}
		migration3 = migration.Migration{Filename: "/migration/3", Version: semver.New(1, 0, 0, "", "")}
	...
	t.Run("One v0.1.0 migration", func(t *testing.T) {
		...
		assert.Equal(t, 1, len(manifest.Migrations["0.1.0"]))
		assert.Equal(t, "/migration/1", manifest.Migrations["0.1.0"][0].Filename)
	})

	t.Run("One v1.0.0 migration", func(t *testing.T) {
		manifest.AddMigration(migration2)
		assert.Equal(t, 1, len(manifest.Migrations["1.0.0"]))
		assert.Equal(t, "/migration/2", manifest.Migrations["1.0.0"][0].Filename)
	})

	t.Run("Two migrations in order", func(t *testing.T) {
		manifest.AddMigration(migration3)
		assert.Equal(t, 2, len(manifest.Migrations["1.0.0"]))
		assert.Equal(t, "/migration/3", manifest.Migrations["1.0.0"][1].Filename)
	})
}
...
func TestMigrator_GenerateManifest(t *testing.T) {
	var (
		...
		correctOrder = []string{"0.0.1", "0.0.2", "0.0.10", "0.1.0", "0.1.1", "0.10.0", "1.0.0"}
	)
	...
	t.Run("Number migrations per version", func(t *testing.T) {
		...
		assert.Equal(t, 2, len(manifest.Migrations["0.0.1"]))
		assert.Equal(t, 2, len(manifest.Migrations["0.0.2"]))
		assert.Equal(t, 1, len(manifest.Migrations["0.0.10"]))
		assert.Equal(t, 1, len(manifest.Migrations["0.1.0"]))
		assert.Equal(t, 3, len(manifest.Migrations["0.1.1"]))
		assert.Equal(t, 2, len(manifest.Migrations["0.10.0"]))
		assert.Equal(t, 3, len(manifest.Migrations["1.0.0"]))
	})

	t.Run("Schema is 0.0.0", func(t *testing.T) {
		// make sure schema is 0.0.0
		assert.Equal(t, "test_manifest/system1/schema.sql", manifest.Migrations["0.0.1"][0].Filename)
		assert.Equal(t, "test_manifest/system2/schema.sql", manifest.Migrations["0.0.1"][1].Filename)
	})

	t.Run("Source order", func(t *testing.T) {
		// spot check migration ordering within versions
		assert.Equal(t, "test_manifest/system1/v0.1.1.sql", manifest.Migrations["0.1.1"][0].Filename)
		assert.Equal(t, "test_manifest/system2/v0.1.1.sql", manifest.Migrations["0.1.1"][1].Filename)
		assert.Equal(t, "test_manifest/system3/v0.1.1.sql", manifest.Migrations["0.1.1"][2].Filename)

		assert.Equal(t, "test_manifest/system1/v1.0.0.sql", manifest.Migrations["1.0.0"][0].Filename)
		assert.Equal(t, "test_manifest/system2/v1.0.0.sql", manifest.Migrations["1.0.0"][1].Filename)
		assert.Equal(t, "test_manifest/system3/v1.0.0.sql", manifest.Migrations["1.0.0"][2].Filename)
	})
}
...
func TestMigrator_GenerateManifestAfterVersion(t *testing.T) {
	...
	t.Run("After schema", func(t *testing.T) {
		// should contain all but schema (v0.0.1) migrations
		manifest, err := migrator.GenerateManifestAfterVersion(semver.New(0, 0, 1, "", ""))
		assert.Nil(t, err)
		totalMigrations = getTotalMigrations(manifest)
	...
	t.Run("Latest only", func(t *testing.T) {
		...
		manifest, err := migrator.GenerateManifestAfterVersion(semver.New(0, 10, 0, "", ""))
		assert.Nil(t, err)
		totalMigrations = getTotalMigrations(manifest)
