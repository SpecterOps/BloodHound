...
func TestDatabase_DeleteAssetGroupTag(t *testing.T) {
	...
	t.Run("cannot delete default decoy tag", func(t *testing.T) {
		databaseInterface := integration.SetupDB(t)

		decoyTags, err := databaseInterface.GetAssetGroupTags(testCtx, model.SQLFilter{SQLString: "type = " + strconv.Itoa(int(model.AssetGroupTagTypeDecoy))})
		require.NoError(t, err)
		require.Len(t, decoyTags, 1)

		err = databaseInterface.DeleteAssetGroupTag(testCtx, testUser, decoyTags[0])
...
func TestDatabase_GetAssetGroupTagForSelection(t *testing.T) {
	...
	tags, err := databaseInterface.GetAssetGroupTagForSelection(testCtx)
	require.NoError(t, err)
