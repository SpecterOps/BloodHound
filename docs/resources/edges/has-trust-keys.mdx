...
The relationship's source node is a domain which has the trust keys for the end node trust account. The trust account exist as the domain has an outbound trust to the domain of the trust account. 
